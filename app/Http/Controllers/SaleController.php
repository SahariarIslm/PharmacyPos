...
class SaleController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        $customers = Customer::orderBy('id', 'DESC')
                ...
                ->where('status', 1)
                ->get();
        $product = BatchStock::orderBy('name', 'ASC')
    ...
    {
        $products = BatchStock::find($request->id);
        \Cart::add(array(
            'id'       => $products->id, 
            'name'     => $products->name,
    ...
    public function item_save(Request $request)
    {
        ...
        foreach (\Cart::getContent() as $item) {
            ...
            $update = DB::table('stocks')
                    ...
                    ->where('batch_no', $item->attributes->batch_no)
                    ->decrement('quantity', $item->quantity);
            $updateBatch = DB::table('batch_stocks')
                    ->where('code', $item->attributes->code)
                    ->where('batch_no', $item->attributes->batch_no)
                    ->decrement('quantity', $item->quantity);
        }
